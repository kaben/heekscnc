...
TEST(ContourTestSuite, testSortEdges2)
{
    /*
    Here's the original stated goal of CContour::SortEdges(): We want
    consistency so that, if we use this Contour operation as a location for
    drilling a relief hole (one day), we want to be sure the machining will
    begin from a consistently known location.

    SortEdges2() doesn't actually meet this goal! Here's a demonstration:
    ...
    However, my goal in SortEdges2() is try to preserve the behavior of
    SortEdges(), which it does.
    */
    gp_Pnt
      pE(5,0,0),
      pENE(4,3,0),
    ...
    wm1.Add(eSE);
    wm1.Add(eESE);
    vector<TopoDS_Edge> edges1(CContour::SortEdges2(wm1.Wire()));
    ...
    wm2.Add(eWNW);
    wm2.Add(eWSW);
    vector<TopoDS_Edge> edges2(CContour::SortEdges2(wm2.Wire()));
